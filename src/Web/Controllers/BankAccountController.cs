...
public class BankAccountController : ControllerBase
{
    ...
    [HttpPost("create")]
    public ActionResult<BankAccount> CreateBankAccount([FromBody] CreateBankAccountRequest createBankAccountRequest)
    {
        if (string.IsNullOrWhiteSpace(createBankAccountRequest.Name))
            throw new AppValidationException("Owner name is required.");

        BankAccount newAccount;
        
        switch (createBankAccountRequest.AccountType)
        {
            case AccountType.Credit:
                if (createBankAccountRequest.CreditLimit == null)
                    return BadRequest("Credit limit is required for a Line of Credit account.");
                newAccount = new LineOfCreditAccount(createBankAccountRequest.Name, createBankAccountRequest.InitialBalance, createBankAccountRequest.CreditLimit.Value);
                break;
            case AccountType.Gift:
                newAccount = new GiftCardAccount(createBankAccountRequest.Name, createBankAccountRequest.InitialBalance, createBankAccountRequest.MonthlyDeposit ?? 0);
                break;
            case AccountType.Interest:
                newAccount = new InterestEarningAccount(createBankAccountRequest.Name, createBankAccountRequest.InitialBalance);
                break;
            default:
